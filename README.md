...
anything from new properties the GDC is seeking to highlight or an old property that has become
more prominently used. All properties not designated either `required` or `preferred` are still
sought by GDC, but submissions without them are allowed. 
