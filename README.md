...
The Blood Profiling Atlas’ (BPA) data dictionary provides the first level of validation for all data
stored in and generated by the BPA. Written in YAML, JSON schemas define all the individual entities
(nodes) in the BPA data model. Moreover, these schemas define all of the relationships (links)
between the nodes. Finally, the schemas define the valid key-value pairs that can be used to
describe the nodes. 

## BPA Data Dictionary Structure 

The BPA Data Model covers all of the nodes within the BPA as well as the relationships between
...
Beyond node type, there are also a number of BPA extensions used to further define the nodes within
...
directed graph of the BPA Data Model.
